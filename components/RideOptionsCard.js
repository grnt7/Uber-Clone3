...
const data = [
    { id: "Uber-X-123", title: "Uber X", multiplier: 1, image: "https://links.papareact.com/3pn" },
    ...
    return (
        <SafeAreaView style={tw`bg-white flex-grow`}>
            <View>
                ...
                <Text style={tw`text-center py-5 text-xl`}>Select A Ride - {travelTimeInformation?.distance?.text || "Calculating..."}</Text>
            </View>
            <FlatList
                style={{ flex: 1 }}
                data={data}
                ...
                renderItem={({ item: { id, title, multiplier, image }, item }) => (
                    ...
                    <TouchableOpacity onPress={() => setSelected(item)} style={tw`flex-row justify-between items-center px-10 ${id === selected?.id && "bg-gray-200"}`}>
                        ...
                        <View style={tw`-ml-6`}>
                            ...
                            <Text>{travelTimeInformation?.duration?.text || "Calculating..."} Travel Time</Text>
                        </View>
